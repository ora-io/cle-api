...
export function compileInner(){
    const commands = [
      `npx asc node_modules/@hyperoracle/zkgraph-lib/main_event_full.ts -o ${innerPrePrePath} --runtime stub --use abort=node_modules/@hyperoracle/zkgraph-lib/common/type/abort --disable bulk-memory --disable mutable-globals --exportRuntime --exportStart ${wasmStartName}  --memoryBase 70000`,
    ];
...
export async function compile(wasmPath, watPath, mappingPath, yamlPath, compilerServerEndpoint, isLocal = false, enableLog=true) {
  ...
  if (isLocal === true) {
    const commands = [
      `npx asc node_modules/@hyperoracle/zkgraph-lib/main_event_local.ts -t ${watPath} -O --noAssert -o ${wasmPath} --disable bulk-memory --use abort=node_modules/@hyperoracle/zkgraph-lib/common/type/abort --exportRuntime --runtime stub --memoryBase 70000`, // note: need --exportRuntime or --bindings esm; (--target release)
    ];
    const combinedCommand = commands.join(" && ");
