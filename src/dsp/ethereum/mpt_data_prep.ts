...
function hexToLittleEndian(hexString: string) {
    let result = hexString.match(/../g).reverse().join('');
    return result;
}

export class MptInputPrep {
    blockCnt: number
    priIpt: string
    ctx: string
    ...
    addBlock2Ctx(blockPrep: BlockPrep) {
        ...
        // account count
        let accCnt = blockPrep.accounts.size
        currCtx += padHexStringToU64LengthLittleEndian(safeHex(accCnt.toString(16)))
        for(let [addr, accData] of blockPrep.accounts) {
            // address
            ...
            // slot count
            let slotCnt = accData.slots.size
            currCtx += padHexStringToU64LengthLittleEndian(safeHex(slotCnt.toString(16)))
            for(let [slotKey, slotData] of accData.slots) {
    ...
    addBlock2PriIpt(blockPrep: BlockPrep) {
        ...
        for(let [addr, accData] of blockPrep.accounts) {
            ...
            let proofStream = "", proofHashStream = ""
            for (let proof of accData.accountProof) {
            ...
            for(let [slotKey, slotData] of accData.slots) {
                ...
                let slotPrfStream = "", slotPrfHashStream = ""
                for (let proof of slotData.storageProof) {
